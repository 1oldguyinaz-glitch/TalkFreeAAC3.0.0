...
function directWord(spec, bucket, index) {
  ...
  if (bucket.visibilityGroup) word.visibilityGroup = bucket.visibilityGroup;
  if (bucket.safetyGate) word.safetyGate = bucket.safetyGate;
...
function navigationWord(child, parent, index) {
  ...
  return {
    ...
    ...(child.visibilityGroup
      ? { visibilityGroup: child.visibilityGroup }
      : {}),
    ...(child.safetyGate ? { safetyGate: child.safetyGate } : {}),
...
function buildColumn(column, rootSpecs) {
  ...
  function build(spec, parent, path, position, inherited = {}) {
    ...
    const safetyGate = spec.safetyGate ?? inherited.safetyGate;
    ...
    if (safetyGate) bucket.safetyGate = safetyGate;
