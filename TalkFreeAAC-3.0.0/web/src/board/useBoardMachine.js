...
export function useBoardMachine(initialStage = 1, initialAgeBand = DEFAULT_AGE_BAND) {
  const [state, dispatch] = useReducer(boardReducer, { stage: initialStage, ageBand: initialAgeBand }, ({ stage, ageBand }) => createInitialBoardState(stage, ageBand));
  const actions = useMemo(() => ({
    ...
    openNestedBucket: (column, item) => dispatch({ type: 'OPEN_NESTED_BUCKET', column, bucketId: item.targetBucketId, bucketLabel: item.label }),
